...
eset_top5 <- eset_sub[order(fData(eset_sub)$pve_fucci, decreasing=T)[1:5],]
...
expr_quant <- do.call(rbind, lapply(1:nrow(counts_normed), function(g) {
  yy <- counts_normed[g,]
  is.zero <- which(yy == 0)
...
title("Predicting cell cycle phase for NA18511", outer=T)
```
